...
A fold indicates that it is finished by returning `Dead` as its `Vitality`.
But there is one more subtlety: When a fold is `Alive`, it can further specify
...
`Ambivalent` will mean the fold *can* accept more input, but doesn't care.
The difference is that tenacious folds keep larger applicative compositions
alive, whereas ambivalent folds do not.
...
being so eager. A demotivated fold will stop receiving input once it becomes
ambivalent.
